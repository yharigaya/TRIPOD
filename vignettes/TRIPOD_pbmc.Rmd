...
according to the accompanying tutorials (see [the preprocessing vignette](http://htmlpreview.github.io/?https://github.com/yharigaya/TRIPOD/blob/main/vignettes/preprocessing_pbmc.html)).
...
* Chromosomal coordinates of target genes (`<dataset_name>.transcripts.gr`)
* ATAC peaks thereof (`<dataset_name>.peaks.gr`)
* Mapping between TFs and motifs (`<dataset_name>.motifxTF`)
* Indicators as to whether a given motif exists in ATAC peaks (`<dataset_name>.peakxmotif`)
...
The preprocessing is described in [the accompanying preprocessing vignette](http://htmlpreview.github.io/?https://github.com/yharigaya/TRIPOD/blob/main/vignettes/preprocessing_pbmc.html).

The following code attaches objects that are necessary for analyzing the PBMC data.

```{r, eval = FALSE}
data(pbmc.transcripts.gr)
data(pbmc.peaks.gr)
data(pbmc.motifxTF)
data(pbmc.peakxmotif)
data(pbmc.metacell.rna)
data(pbmc.metacell.peak)
data(pbmc.metacell.celltype)
data(pbmc.metacell.celltype.col)
```
...
TRIPOD matching $X_t$ for the *GNLY* gene.

```{r, eval = TRUE}
xymats.tripod.Xt <- xymats.tripod.Xt.list$GNLY
...
through a process described in [the preprocessing vignette](http://htmlpreview.github.io/?https://github.com/yharigaya/TRIPOD/blob/main/vignettes/preprocessing_pbmc.html).
...
delta.coeff.pval <- deleteCellType(Yg = Yg, Xt = Xt, Yj = Yj,
	metacell.celltype = pbmc.metacell.celltype, alternative = "less")
```

The following code generates a plot of the $p$-values for the coefficient of
the interaction term. We use a significance threshold adjusted for the number of
cell types tested.

```{r, eval = TRUE}
ordered.celltype <- levels(pbmc$celltype)
# delta.coeff.pval <- delta.coeff.pval[ordered.celltype, ]
...
delta.coeff.pval <- deleteBranch(Yg = Yg, Xt = Xt, Yj = Yj, dend = dend,
	alternative = "less", metacell.celltype = pbmc.metacell.celltype)
plotTree(dend = dend, delta.coeff.pval = delta.coeff.pval,
	metacell.celltype = pbmc.metacell.celltype, coefficient = "Xt:Yj",
...
delta.coeff.pval <- deleteNeighbor(Yg = Yg, Xt = Xt, Yj = Yj,
	metacell.celltype = pbmc.metacell.celltype, metacell.neighbors = metacell.neighbors,
	alternative = "less")
