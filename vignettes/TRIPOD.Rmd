...
gene expression using sigle-cell multiomic data, which is publicly availabe from 
...
$i$ by $t$ matrix, $X$, where $i$ is the number of cells and $t$ is the number of peaks.
$X_{it}$ represents the $(i, t)$ element.
The processed RNA-seq data can be represented by a $i$ by $g$ matrix, $Y$, 
where $i$ is the number of cells and $g$ is the number of genes.
$Y_{ig}$ represents the $(i, g)$ element.
Importantly, $i$ is shared between $X$ and $Y$ because of the multiomic technology.
...
In most cases, we include peaks 200 Kb up and downstream from TSS.
We also consider RNA expression levels of a set of TFs, $Y_ij$, 
...
The data preprocessing is performed using the Seurat and Signac packages (ref)
according to the accompanying tutorials (see the preprocessing vignette).
...
(Section 4). `xymats` is a list containing information that is necessary to
...
columns representing different peaks and TFs, respectively.
The `xymats` object is also used as input for RNA prediction (Section 5).
For the influential analysis, we first extract the $Y_g$, $X_t$, and $Y_j$ vectors
for a given trio (Section 6).
...
as well as associated $p$-values.
...
* By default, `fitModel()` removes outliers from the `xymats$Yg`, `xymats$Xt`, 
and `xymats$Yj` elements. That is, the elements differ between the input and output.

Please refer to our manuscript (link) for formulation of the models.

# 4 Required package(s)

The following package, which is available from (URL), is required to run this vignette.
...
The preprocessing is described in the accompanying preprocessing vignette.
...
$Y_g$, $X_t$, and $Y_j$. Here, we obtain a list of the lists for the four genes.
...
See our manuscript (link) for details.
...
(Is it useful to list the coefficient names for all models?)
...
We select hits by Benjamini-Hochberg method with a certain FDR threshold.
...
The `getTrios()` function returns a data frame.

```{r, eval = TRUE}
# marginal
xymats.m.pos.df <- getTrios(
	xymats.list = xymats.m.list,
	fdr.thresh = fdr.thresh,
	sign = sign,
  model.name = "marginal"
)
head(xymats.m.pos.df[order(xymats.m.pos.df$adj), ])
```

```{r, eval = TRUE}
# conditional
xymats.cY.pos.df <- getTrios(
	xymats.list = xymats.c.list,
	fdr.thresh = fdr.thresh,
	sign = sign,
  model.name = "conditional.on.Yj"
)
head(xymats.cY.pos.df[order(xymats.cY.pos.df$adj), ])
```

```{r, eval, TRUE}
xymats.cX.pos.df <- getTrios(
	xymats.list = xymats.c.list,
	fdr.thresh = fdr.thresh,
	sign = sign,
  model.name = "conditional.on.Xt"
)
head(xymats.cX.pos.df[order(xymats.cX.pos.df$adj), ])
```
...
head(xymats.i.pos.df[order(xymats.i.pos.df$adj), ])
...
using the `plotTFPeakGeneScatter()` function.
In this section, we focus on the regulation of the *Sox10* gene by the Olig2 TF
...
For this purpose, we first load the Seurat object.

```{r, include = FALSE, eval = TRUE, message = FALSE, warning = FALSE}
# for testing on my local machine only
file <- "e18.6.20210811.rds"
dir.data <- "~/Dropbox/jiang_lab/project_alignment/data/data_011/preprocess_20210811"
...
the linear interaction model due to the large computational burdens of fitting
the nonparametric models and performing the sampling procedure in this analysis
(Or are there more theoretical reasons?).

## 7.1 Preparation

We focus on the regulation of the regulation of the *Neurog2*
...
that the deletion of metacells of a given cell type decreases the coefficients
...
The above results implies that the regulation of the *Neurog2*
...
PBMC dataset. Please refer to our manuscript for those examples.
...
each of the metacells first. The `getMetacellNeighbor()` function, which
obtain k nearest neighbors for each point, can be used for
this purpose. In the following code, we set $k$ to 4.

```{r, eval = TRUE}
# need the Seurat object
reduction <- "umap.rna"
