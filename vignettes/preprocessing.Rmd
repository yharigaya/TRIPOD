...
author: "Yuriko Harigaya, Nancy Zhang, Yuchao Jiang"
...
This is a supplement to the TRIPOD vignette.
Here, we demonstrate data preprocessing required for conducting
analysis using the TRIPOD package.
The input is sigle-cell multiomic data, which is publicly available from 
...
# 2 Required package(s)

The TRIPOD package, which is available from (URL), is required to run this vignette.

```{r install, eval = FALSE}
# this does not work yet
install.package(TRIPODprep) # install_github()
```
...
The input data (`e18_mouse_brain_fresh_5k_filtered_feature_bc_matrix.h5` and
`e18_mouse_brain_fresh_5k_atac_fragments.tsv.gz`) can be can be downloaded 
...
dir.data <- "~/Dropbox/multiome_shared_snapshots/EmbryonicMouseBrain_10x_multiomics/e18_mouse_brain_fresh"
...
Although TRIPOD does not strictly require cell type labels, it can substantially enrich 
...
path <- "~/Dropbox/jiang_lab/project_alignment/data/data_011/preprocess_20210623/e18.1.20210623.rds" 
...
We illustrate merely an example case below, in which our basic goals
are as follows.
...
e18 <- RunChromVAR( # note that this step can take 30-60 minutes
  object = e18,
...
The `transcripts.gr` object is a GRanges object from the GenomicRanges package
...
The `motifxTF` object is a matrix (change to data.frame?) containing mapping
...
```{r, eval = TRUE, message = FALSE, warning = FALSE}
# peak by motif: overlap of ATAC peaks and motifs (binary)
...
# may need to use a more formal method
# i assume that the results would be very similar or identical
...
# e18 <- FindClusters(e18, resolution = 0.8, verbose = FALSE)
```
